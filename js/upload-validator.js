...
const uploadImageDescriptionElement =
...
const isValidLength = (value) => {
  ...
  const arrayOfMessages = value.trim().split(/\s+/);
  return arrayOfMessages.every(
    (item) => item.length > 1 && item.length < MAX_HASHTAGS_LENGTH
  );
};
...
const findSameElements = (value) => {
  ...
  return arrayOfMessages.every((item, index, array) =>
    array.slice(index + 1, array.length).every((elem) => elem !== item)
  );
};
...
const isValid = () => {
  pristine.addValidator(
    ...
    validateHashtagContent,
    'Хэш-тег должен начинаться с символа # и состоять из букв и цифр'
  );
  pristine.addValidator(
    hashtagsInputElement,
    isValidLength,
    'Длина хеш-тега должна быть от 2 до 20 символов'
  );
  pristine.addValidator(
    hashtagsInputElement,
    findSameElements,
  ...
  pristine.addValidator(
    uploadImageDescriptionElement,
    (text) => text.length < MAX_IMAGE__DESCRIPTION__LENGTH,
